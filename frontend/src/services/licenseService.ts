...
import { TRIAL_HOURS, DAILY_LIMIT } from '../constants';
import { LicenseStatus } from '../types';
import { LicenseApiService } from './licenseApiService';

const DISABLE_USAGE_LIMITS_FOR_TESTING = true;
...
export const LicenseService = {
  ...
  getDailyRemaining: (): number => {
    if (DISABLE_USAGE_LIMITS_FOR_TESTING) {
      return 999;
    }
  ...
  getLicenseStatus: (): LicenseStatus => {
    if (DISABLE_USAGE_LIMITS_FOR_TESTING) {
      return { status: 'active_pro', label: 'Pro 会员', type: 'lifetime' };
    }
  ...
  verifyLicense: async (inputCode: string) => {
    ...
    const result = await LicenseApiService.verifyLicense({ machineId, code });

    if (result.success && result.valid) {
      if (result.type === 'subscription' && result.expireDate) {
        localStorage.setItem('grammar_is_activated', 'true');
        localStorage.setItem('grammar_license_type', 'subscription');
        localStorage.setItem('grammar_expire_date', result.expireDate);
        return { success: true, expireDate: result.expireDate };
    ...
    if (result.reason === 'expired') throw new Error("此激活码已过期。");
